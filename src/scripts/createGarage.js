...
const garage = []
  ...
  const createGarage = function() {
    return {
      ...
      //.find loops thru array until it finds the first item that matches specified condition
      //car represents each item as we iterate thru it
      //in main.js, looking for sierra - each model is false until sierra
      retrieve(carToFind) {
